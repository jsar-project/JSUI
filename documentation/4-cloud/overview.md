...
当前云端集成覆盖三类能力：平台官方智能体可读取账号 Token；服务端可按账号和智能体缓存一次性消息；第三方系统可向指定用户推送通知，并在用户点击后打开已注册的 AIUI 页面。
...
- [获取账号 Token](./account-token.md)：平台官方智能体的账号接口。
- [缓存 AIUI 消息](./message-cache.md)：10 分钟、覆盖式、一次性消费的消息缓存。
